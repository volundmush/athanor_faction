...
VERSION_PATH = os.path.join("athanor_faction", "VERSION.txt")
...
def package_data():
    ...
    """
    file_set = []
    for root, dirs, files in os.walk("athanor_faction"):
        for f in files:
            if ".git" in f.split(os.path.normpath(os.path.join(root, f))):
                # Prevent the repo from being added.
                continue
            file_name = os.path.relpath(os.path.join(root, f), "athanor_faction")
...
# setup the package
setup(
    name="athanor_faction",
    version=get_version(),
    author="Volund",
    maintainer="Volund",
    url="https://github.com/volundmush/athanor_faction",
    ...
    python_requires=">=3.7",
    project_urls={
        "Source": "https://github.com/volundmush/athanor_faction",
